	...
	! This program addresses the problem of dispersion of an initial mound.
	! Initial conditions are given as a Gussian hump.
	! (see H. Lamb, 1932)
	...
	call input (imax,dx,cf,g,dt,rho,tend,zeta,hs,h,q,x)
...
500 continue
    ...
    open(3,file='zeta_output.dat',status='old')
    open(4,file='q_output.dat',status='old')
	...
	end program SOBEY_MOUND

!***************************************************************

	subroutine input (imax,dx,cf,g,dt,rho,tend,zeta,hs,h,q,x)
    implicit none
	double precision :: pi,lx,g,h0,dx,cf,dt,rho,tend
	double precision :: amp,bx,cx,x(imax)
	...
	lx=3000.0d0									! domain length
	cf=0.0d0									! bed friction coefficient
    h0=100.0d0									! initial water depthy 
    amp=5.0d0  									! initial disturbance
    bx=50.0d0									! initial half width
    cx=log(real(2.0d0))	! LOG(2)
    ...
    tend=dt! 10.0d0								! duration of simulation
    !
    !   initial values of free surface elevation, still water depth
    !   total depth and flux
    !
	do i=1,imax
 	x(i)=dble(-700.0d0+i)*dx 					! distance along bed
  	zeta(i)=amp*exp(-cx*(x(i)/bx)*(x(i)/bx)) 	! initial free surface elevation
...
!***************************************************************
    ...
    ! For now assume wave(II) (Sobey,p.6)
